...
NUM_STEPS = 10582 # Number of images in the validation set.
...
def main():
    ...
    raw_output = tf.argmax(raw_output_up, dimension=3)
    ...
    preds_entropy = np.zeros(args.num_steps)
    ...
    if os.path.isfile(SAVE_LIST_PATH):
        print('File is already present. Deleting it...')
        os.remove(SAVE_LIST_PATH)

    hard_samples_list = open(SAVE_LIST_PATH, "w")

    for i in range(args.num_steps):
        index = indices[i]
        img = image_names[index]
        hard_samples_list.write(img)

    hard_samples_list.close()

    print('Hard samples list is saved at: %s'%(SAVE_LIST_PATH))
