...
def image_scaling(img, label, activation):
    """
    ...
    Args:
      ...
      activation: Activation mask to scale.
    """
    ...
    activation = tf.image.resize_images(activation, new_shape)
    label = tf.image.resize_nearest_neighbor(tf.expand_dims(label, 0), new_shape)
    label = tf.squeeze(label, squeeze_dims=[0])
   
    return img, label, activation

def image_mirroring(img, label, activation):
    """
    ...
    Args:
      ...
      activation: Activation mask to mirror.
    """
    ...
    activation = tf.reverse(activation, mirror)
    return img, label, activation

def random_crop_and_pad_image_and_labels(image, label, activation, crop_h, crop_w, ignore_label=255):
    """
    ...
    combined = tf.concat(2, [image, label, activation]) 
    ...
    last_actv_dim = tf.shape(activation)[-1]
    combined_crop = tf.random_crop(combined_pad, [crop_h,crop_w,25])
    img_crop = combined_crop[:, :, :last_image_dim]
    label_crop = combined_crop[:, :, last_image_dim:last_image_dim+last_label_dim]
    activation_crop = combined_crop[:, :, last_image_dim+last_label_dim:]
    ...
    activation_crop.set_shape((crop_h, crop_w, 21))
    return img_crop, label_crop, activation_crop  
...
def read_images_from_disk(input_queue, input_size, random_scale, random_mirror): # optional pre-processing arguments
    ...
    activation = np.load(input_queue[2]) #TODO: Need to check if this works?
    ...
    if input_size is not None:
        ...
        if random_scale:
            img, label, activation = image_scaling(img, label, activation)

        # Randomly mirror the images and labels.
        if random_mirror:
            img, label, activation = image_mirroring(img, label, activation)

        # Randomly crops the images and labels.
        img, label, activation = random_crop_and_pad_image_and_labels(img, label, activation, h, w, IGNORE_LABEL)
...
class ImageReader_Distill(object):
    ...
    def __init__(self, data_dir, actv_dir, data_list, input_size, random_scale,
                 random_mirror, coord):
